    ...
    def generate(self, _curdir, report=None, localization=None):
        ...
        for filename in os.listdir(report):
            ...
            if localization:
                ...
                # df[len(df.columns)], df[len(df.columns)+1] = izip(*df.iloc[:,-1].apply(lambda x: self.index_lookup(x, localization_k_v)))
                df['existing_eys'], df['existing_localization_file'] = izip(*df.iloc[:,-1].apply(lambda x: self.index_lookup(x, localization_k_v)))
                # add existing keys to report
            ...
            # names = ['filename', 'startLineNumber', 'startCharIdx', 'endLineNumber', 'endCharIdx', 'text', 'existing_key', 'existing_localization_file', 'generated_key']
            print(df)
